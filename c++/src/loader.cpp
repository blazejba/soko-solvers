...
std::vector<Maze> readMazes(const std::string& filename) {
    ...
    while (file) {
        ...
        if (!file) break;  // If we couldn't read the headers, we're probably at the end of the file

        maze.length = static_cast<int>(length);
...
int main() {
    std::string filename = "../../maps/mazes.bin";
    std::vector<Maze> mazes = readMazes(filename);

    // Print the mazes to check
    for (const Maze& maze : mazes) {
        std::cout << "Length: " << maze.length << ", Size X: " << maze.size_x << ", Size Y: " << maze.size_y << std::endl;
